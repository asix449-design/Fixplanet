...
| `/migration` | Hub tiles only (Humans / Birds / Animals). Human dispersal encyclopedia + map on `/migration/humans`. Bird flyway schematic and species cards on `/migration/birds`. Mammal migrations on `/migration/animals`. Detail pages at `/migration/[slug]` |
...
To add a Migration card: add a row in `src/data/migration.ts` (English slug, shelf `birds` | `animals`, optional Latin name, image credit, primary source URLs), the same slug in all four language objects under `src/i18n/migration-en.ts` (and `-ru` / `-pl` / `-lv`), and a Wikimedia/PD photo in `public/images/migration/` recorded in `credits.json`. Hub tiles are `/migration/humans`, `/migration/birds`, `/migration/animals`; `?shelf=` on the hub redirects. Do not invent routes or dates. Prefer BirdLife / CMS flyways, IUCN, NOAA, and named paleodemography papers. Drivers belong on each entry. Megafauna lists stay on Wildlife.
