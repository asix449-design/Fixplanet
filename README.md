...
| `/migration` | Hub tiles (Today / Humans / Great migrations / Birds / Animals). Current-day UN-region net-migration map on `/migration/today`. Era-map scrubber + small medieval commoner on `/migration/humans`. Historic mass movements on `/migration/great-migrations` (Huns first). Bird flyways on `/migration/birds`. Mammal migrations on `/migration/animals`. Detail pages at `/migration/[slug]` |
...
To add a Migration card: add a row in `src/data/migration.ts` (English slug, shelf `great-migrations` | `birds` | `animals`, optional Latin name, image credit, primary source URLs), the same slug in all four language objects under `src/i18n/migration-en.ts` (and `-ru` / `-pl` / `-lv`), and a Wikimedia/PD photo in `public/images/migration/` recorded in `credits.json`. Hub tiles are `/migration/today`, `/migration/humans`, `/migration/great-migrations`, `/migration/birds`, `/migration/animals`; `?shelf=` on the hub redirects. The Today shelf is a current-day map: commit curated UN figures in `public/data/migration-today.json` (WPP net migration year + IMS stock year). Do not scrape a live API. Do not invent annual arrived/left splits; label net and stock plainly. Do not invent routes or dates. Prefer BirdLife / CMS flyways, IUCN, NOAA, named paleodemography papers, and named late-antique sources. Great-migrations cards are historic mass movements — keep them off the Pleistocene Humans shelf. The Humans shelf is map-first: named migration events in `src/data/human-migration-events.ts` (dated movements with sourced plates or teaching schematics; not a 50-year grid and no invented global population choropleths). Drivers belong on each entry. Megafauna lists stay on Wildlife.
