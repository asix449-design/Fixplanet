...
| `/migration` | Hub tiles (Humans / Great migrations / Birds / Animals). Era-map scrubber + small medieval commoner on `/migration/humans`. Historic mass movements on `/migration/great-migrations` (Huns first). Bird flyways on `/migration/birds`. Mammal migrations on `/migration/animals`. Detail pages at `/migration/[slug]` |
