...
| `/migration` | Hub tiles (Humans / Great migrations / Birds / Animals). Late Pleistocene dispersal + map on `/migration/humans`. Historic mass movements on `/migration/great-migrations` (Huns first). Bird flyways on `/migration/birds`. Mammal migrations on `/migration/animals`. Detail pages at `/migration/[slug]` |
