...
| `/geoengineering` | Failure-honest catalog of landscape megaprojects (water diversion, desert greening) plus a smaller climate-intervention shelf. Hub tiles then cards; detail pages at `/geoengineering/[slug]`; shelf URLs `/geoengineering/water`, `/desert`, `/climate` and `?shelf=`. Ochre / carved-earth theme. Not a climate-hype page |
...
To add a geoengineering project: add a row in `src/data/geoengineering.ts` (English slug, shelf `water` | `desert` | `climate`, status `proposed` | `partial` | `built-struggling` | `failed` | `ongoing` | `research`, years, image credit, primary source URLs), the same slug in all four language objects under `src/i18n/geoengineering-en.ts` (and `-ru` / `-pl` / `-lv`), and a Wikimedia/PD image in `public/images/geoengineering/` recorded in `credits.json`. Shelf hub tiles are `/geoengineering/water`, `/geoengineering/desert`, `/geoengineering/climate`; `?shelf=` also works. Do not invent hectare counts, collapse dates, or failure stories. Prefer “partial success with costs” when evidence is mixed. Climate-intervention cards stay on the climate shelf and must read as research/proposal, not a ready planetary fix. Keep Saudi Green Initiative, historical groundwater wheat, and NEOM as separate programmes.
